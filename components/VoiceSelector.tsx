...
const VoiceSelector: React.FC<VoiceSelectorProps> = ({
  ...
  return (
    <div className="relative">
      ...
      >
          <span className="text-base sm:text-lg md:text-xl flex-shrink-0">{currentVoiceOption.icon}</span>
          <div className="flex flex-col items-start hidden sm:flex min-w-0">
            <span className="font-display text-[10px] sm:text-xs font-semibold text-slate-300 group-hover:text-white transition-colors truncate">
              Voix: {currentVoiceOption.name}
            </span>
          </div>
        <svg 
          className={`w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-400 group-hover:text-white transition-all duration-300 flex-shrink-0 ${isOpen ? 'rotate-180' : ''}`} 
          fill="none" 
          viewBox="0 0 24 24" 
          stroke="currentColor"
        >
      ...
      {isOpen && (
        <>
          ...
          {/* Liste des voix */}
          <div 
            className="absolute top-full mt-2 right-0 w-[calc(100vw-1rem)] sm:w-64 md:w-72 max-w-[calc(100vw-1rem)] sm:max-w-none glass-intense rounded-xl sm:rounded-2xl border border-white/10 overflow-hidden z-50 animate-in"
            style={{
          ...
          >
            <div className="p-1.5 sm:p-2 space-y-1">
              {AVAILABLE_VOICES.map((voice) => (
                ...
                >
                  <span className="text-xl sm:text-2xl flex-shrink-0">{voice.icon}</span>
