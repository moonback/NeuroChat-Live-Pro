...
export const ActionLogViewer: React.FC = () => {
    const { actionLogs, isLogsModalOpen, setLogsModalOpen, clearActionLogs } = useAppStore();
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 sm:p-6 transition-all duration-300">
            <div className="bg-slate-900 border border-slate-700/50 shadow-2xl rounded-2xl w-full max-w-3xl max-h-[85vh] flex flex-col overflow-hidden ring-1 ring-white/10">
                <div className="flex items-center justify-between p-4 border-b border-slate-800 bg-slate-900/80">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-semibold text-white tracking-tight">Logs d'Actions IA</h2>
                            <p className="text-xs text-slate-400">Historique des outils exécutés par l'assistant</p>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-900/50 custom-scrollbar">
                    {actionLogs.length === 0 ? (
                        ...
                        actionLogs.map((log) => (
                            <div
                                key={log.id}
                                className={`p-3 rounded-xl border transition-colors ${log.result === 'success' ? 'bg-green-500/5 border-green-500/10 hover:border-green-500/20' :
                            ...
                            >
                                ...
                                {Object.keys(log.args).length > 0 && (
                                    <div className="bg-black/40 rounded-lg p-2 mt-2 border border-white/5">
